...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_exp2_process_check_sum_call( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_exp2_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_exp2_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_exp2_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_exp2_get_eml_resolved_functions_info( int nlhs, mxArray
    * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "exp2") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_exp2_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_exp2_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"exp2")) {
    return(sf_exp2_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
...
extern void exp2_register_exported_symbols(SimStruct* S);
extern void exp2_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  exp2_register_exported_symbols(S);
}
