...
static SFc3_minimalistInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c3_minimalist(void);
static void initialize_params_c3_minimalist(void);
static void enable_c3_minimalist(void);
static void disable_c3_minimalist(void);
static void c3_update_debugger_state_c3_minimalist(void);
static const mxArray *get_sim_state_c3_minimalist(void);
static void set_sim_state_c3_minimalist(const mxArray *c3_st);
static void finalize_c3_minimalist(void);
static void sf_c3_minimalist(void);
...
static void initialize_c3_minimalist(void)
{
  ...
  chartInstance.c3_is_active_c3_minimalist = 0U;
}
...
static const mxArray *get_sim_state_c3_minimalist(void)
{
  ...
  c3_b_u = chartInstance.c3_is_active_c3_minimalist;
  c3_c_y = NULL;
...
static void set_sim_state_c3_minimalist(const mxArray *c3_st)
{
  ...
  *c3_c_byte = c3_y;
  ...
  sf_mex_import("is_active_c3_minimalist", sf_mex_dup(c3_b_byte), &c3_u1, 1, 3,
                0U, 0, 0U, 0);
  c3_b_y = c3_u1;
  sf_mex_destroy(&c3_b_byte);
  chartInstance.c3_is_active_c3_minimalist = c3_b_y;
  sf_mex_destroy(&c3_u);
  c3_update_debugger_state_c3_minimalist();
...
const mxArray *sf_c3_minimalist_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c3_minimalist_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c3_minimalist_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c3_minimalist(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[8],T\"byte\",},{M[8],M[0],T\"is_active_c3_minimalist\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c3_minimalist_get_check_sum(&mxChecksum);
...
static void sf_opaque_initialize_c3_minimalist(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c3_minimalist();
  initialize_c3_minimalist();
}

static void sf_opaque_enable_c3_minimalist(void *chartInstanceVar)
{
  enable_c3_minimalist();
}

static void sf_opaque_disable_c3_minimalist(void *chartInstanceVar)
{
  disable_c3_minimalist();
}

static void sf_opaque_gateway_c3_minimalist(void *chartInstanceVar)
{
  sf_c3_minimalist();
}

static mxArray* sf_opaque_get_sim_state_c3_minimalist(void *chartInstanceVar)
{
  mxArray *st = (mxArray *) get_sim_state_c3_minimalist();
  return st;
}

static void sf_opaque_set_sim_state_c3_minimalist(void *chartInstanceVar, const
  mxArray *st)
{
  set_sim_state_c3_minimalist(sf_mex_dup(st));
}

static void sf_opaque_terminate_c3_minimalist(void *chartInstanceVar)
{
  ...
  finalize_c3_minimalist();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c3_minimalist(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c3_minimalist(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("minimalist","minimalist",3);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop("minimalist","minimalist",3,"RTWCG"));
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"minimalist","minimalist",3,1);
      sf_mark_chart_reusable_outputs(S,"minimalist","minimalist",3,1);
    }

    sf_set_rtw_dwork_info(S,"minimalist","minimalist",3);
...
static void mdlRTW_c3_minimalist(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "minimalist", "minimalist",3);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c3_minimalist(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway = sf_opaque_gateway_c3_minimalist;
  chartInstance.chartInfo.initializeChart = sf_opaque_initialize_c3_minimalist;
  chartInstance.chartInfo.terminateChart = sf_opaque_terminate_c3_minimalist;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c3_minimalist;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c3_minimalist;
  chartInstance.chartInfo.getSimState = sf_opaque_get_sim_state_c3_minimalist;
  chartInstance.chartInfo.setSimState = sf_opaque_set_sim_state_c3_minimalist;
  chartInstance.chartInfo.getSimStateInfo = sf_get_sim_state_info_c3_minimalist;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c3_minimalist;
  chartInstance.chartInfo.mdlStart = mdlStart_c3_minimalist;
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c3_minimalist;
...
void c3_minimalist_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c3_minimalist(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c3_minimalist(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c3_minimalist(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c3_minimalist_method_dispatcher.\n"
