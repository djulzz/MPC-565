...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_minimalist_rep_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_minimalist_rep_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_minimalist_rep_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_minimalist_rep_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_minimalist_rep_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "minimalist_rep") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_minimalist_rep_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_minimalist_rep_method_dispatcher(SimStruct *S, unsigned
...
{
  if (!strcmp(machineName,"minimalist_rep")) {
    return(sf_minimalist_rep_method_dispatcher(simstructPtr,chartFileNumber,
            method,data));
  }
...
extern void minimalist_rep_terminator(void);
...
extern void minimalist_rep_initializer(void);
extern void minimalist_rep_register_exported_symbols(SimStruct* S);
extern void minimalist_rep_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  minimalist_rep_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      minimalist_rep_debug_initialize();
    }

    minimalist_rep_initializer();
  }
