...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_working_one1_MPC_removed_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_working_one1_MPC_removed_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_working_one1_MPC_removed_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_working_one1_MPC_removed_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_working_one1_MPC_removed_get_eml_resolved_functions_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "working_one1_MPC_removed") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_working_one1_MPC_removed_get_eml_resolved_functions_info(nlhs,plhs,
      2,newRhs);
  }
...
extern unsigned int sf_working_one1_MPC_removed_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"working_one1_MPC_removed")) {
    return(sf_working_one1_MPC_removed_method_dispatcher(simstructPtr,
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    working_one1_MPC_removed_terminator();
    sf_debug_terminate();
  }
...
extern void working_one1_MPC_removed_register_exported_symbols(SimStruct* S);
extern void working_one1_MPC_removed_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  working_one1_MPC_removed_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      working_one1_MPC_removed_debug_initialize();
    }

    working_one1_MPC_removed_initializer();
  }
