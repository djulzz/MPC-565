  ...
  nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Subsystem_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Subsystem_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
  ...
  char *machineName, unsigned int chartFileNumber, int_T method, void *data)
{
  if (!strcmp(machineName,"Subsystem")) {
    return(sf_Subsystem_method_dispatcher(simstructPtr,chartFileNumber,method,
            data));
  }
...
extern void Subsystem_debug_initialize(void);
