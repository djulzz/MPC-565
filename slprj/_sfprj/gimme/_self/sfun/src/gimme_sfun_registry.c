...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_gimme_process_check_sum_call( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_gimme_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_gimme_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_gimme_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_gimme_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "gimme") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_gimme_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_gimme_method_dispatcher(SimStruct *S, unsigned int
...
{
  if (!strcmp(machineName,"gimme")) {
    return(sf_gimme_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }

  return 0;
}

extern void gimme_terminator(void);
...
extern void gimme_initializer(void);
extern void gimme_register_exported_symbols(SimStruct* S);
extern void gimme_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  gimme_register_exported_symbols(S);
}
