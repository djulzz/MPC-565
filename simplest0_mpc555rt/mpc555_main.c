...
#include "simplest0.h"                 /* Model's header file */
#include "profile.h"
...
void rt_OneStep(MPC555_IRQ_LEVEL level)
{
  ...
  if (OverrunFlags[0] > BASE_RATE_MAX_OVERRUNS) {
    rtmSetErrorStatus(simplest0_M, "Overrun");
  }
   ...
   *************************************************/
  if (rtmGetErrorStatus(simplest0_M) != NULL) {
    return;
  }
...
#define LATCH_0_31_IMB_INTERRUPT_LEVELS 3                        /* Enable all IRQ levels on the UIMB */
  ...
  EIE();
  while (rtmGetErrorStatus(simplest0_M) == NULL) {
