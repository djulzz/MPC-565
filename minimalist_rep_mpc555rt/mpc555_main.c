...
#include "minimalist_rep.h"            /* Model's header file */
#include "profile.h"
...
void rt_OneStep(MPC555_IRQ_LEVEL level)
{
  ...
  if (OverrunFlags[0] > BASE_RATE_MAX_OVERRUNS) {
    rtmSetErrorStatus(minimalist_rep_M, "Overrun");
  }
   ...
   *************************************************/
  if (rtmGetErrorStatus(minimalist_rep_M) != NULL) {
    return;
  }
...
#define LATCH_0_31_IMB_INTERRUPT_LEVELS 3                        /* Enable all IRQ levels on the UIMB */
  ...
  EIE();
  while (rtmGetErrorStatus(minimalist_rep_M) == NULL) {
