...
#include "Simplest_With_For_Iterator5.h" /* Model's header file */
#include "profile.h"
...
void rt_OneStep(MPC555_IRQ_LEVEL level)
{
  ...
  if (OverrunFlags[0] > BASE_RATE_MAX_OVERRUNS) {
    rtmSetErrorStatus(Simplest_With_For_Iterator5_M, "Overrun");
  }
   ...
   *************************************************/
  if (rtmGetErrorStatus(Simplest_With_For_Iterator5_M) != NULL) {
    return;
  }
  ...
  while (OverrunFlags[0] > 0 ) {
     ...
     * Step model *
     **************/
    Simplest_With_For_Iterator5_step();
...
#define LATCH_0_31_IMB_INTERRUPT_LEVELS 3                        /* Enable all IRQ levels on the UIMB */
  ...
  /* Initialize model */
  Simplest_With_For_Iterator5_initialize(1);
  ...
  EIE();
  while (rtmGetErrorStatus(Simplest_With_For_Iterator5_M) == NULL) {
