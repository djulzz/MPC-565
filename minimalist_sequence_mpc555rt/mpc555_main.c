...
#include "minimalist_sequence.h"       /* Model's header file */
#include "profile.h"
...
void rt_OneStep(MPC555_IRQ_LEVEL level)
{
  ...
  if (OverrunFlags[0] > BASE_RATE_MAX_OVERRUNS) {
    rtmSetErrorStatus(minimalist_sequence_M, "Overrun");
  }
   ...
   *************************************************/
  if (rtmGetErrorStatus(minimalist_sequence_M) != NULL) {
    return;
  }
...
#define LATCH_0_31_IMB_INTERRUPT_LEVELS 3                        /* Enable all IRQ levels on the UIMB */
  ...
  /* Initialize model */
  minimalist_sequence_initialize(1);
  ...
  EIE();
  while (rtmGetErrorStatus(minimalist_sequence_M) == NULL) {
