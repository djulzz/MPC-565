...
#include "then_rep.h"                  /* Model's header file */
#include "profile.h"
...
void rt_OneStep(MPC555_IRQ_LEVEL level)
{
  ...
  if (OverrunFlags[0] > BASE_RATE_MAX_OVERRUNS) {
    rtmSetErrorStatus(then_rep_M, "Overrun");
  }
   ...
   *************************************************/
  if (rtmGetErrorStatus(then_rep_M) != NULL) {
    return;
  }
...
#define LATCH_0_31_IMB_INTERRUPT_LEVELS 3                        /* Enable all IRQ levels on the UIMB */
  ...
  EIE();
  while (rtmGetErrorStatus(then_rep_M) == NULL) {
